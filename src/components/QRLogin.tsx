...
export default function QRLogin({ onClose, onLoggedIn }: Props) {
  ...
  const [status, setStatus] = useState<'loading' | 'waiting' | 'scanned' | 'expired' | 'success'>(
    'loading'
  );
  ...
  useEffect(() => {
    if (!key || status === 'success' || status === 'expired') return;
        ...
        <div className="flex justify-center">
          <div className="relative w-52 h-52 bg-white rounded-2xl p-3 flex items-center justify-center overflow-hidden">
            ...
            {status === 'expired' && (
              <button
                onClick={initQr}
                className="absolute inset-0 flex items-center justify-center bg-black/10 text-black text-sm font-semibold"
              >
                点击刷新
              </button>
            )}
